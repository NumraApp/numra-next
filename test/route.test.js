...
/* No Next.js here on purpose. The package imports nothing from next — a route
   handler is a function taking a Request and returning a Response — so the
   tests drive it with the platform's own globals. Booting Next to prove that
   would test Next, not this. */

const SECRET = 'whsec_test';

async function build(opts = {}, upstreamHandler = () => ({ body: LOOKUP_OK })) {
  const upstream = await startMockServer(upstreamHandler);
  const { POST } = createNumraRoute({
    client: new Numra({ apiKey: 'k', baseUrl: upstream.url }),
    ...opts,
  });
  return { POST, upstream, close: () => upstream.close() };
}

const call = (POST, action, body, headers = {}) =>
  POST(
    new Request(`http://localhost/api/numra/${action}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', ...headers },
      body: typeof body === 'string' ? body : JSON.stringify(body),
    }),
    { params: Promise.resolve({ numra: [action] }) },
  );
...
test('with no authorize, every request is refused as a misconfiguration', async () => {
  ...
  try {
    t = await build();
    res = await call(t.POST, 'check', { phone: '+212600000000' });
  } finally {
    console.error = real;
  }
...
test('a rejecting authorize is a plain 403', async () => {
  ...
  const res = await call(t.POST, 'check', { phone: '+212600000000' });
...
test('the segment name does not matter', async () => {
  /* The action is read from params, not from the URL, so `[...slug]` works
     as well as `[...numra]` and the route survives being moved. */
  const t = await build({ authorize: () => true });
  const res = await t.POST(
    new Request('http://localhost/whatever/they/named/it', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ phone: '+212600000000' }),
    }),
    { params: { slug: ['deeply', 'nested', 'check'] } },
  );
...
test('an unknown action is a 404, not a lookup', async () => {
  const t = await build({ authorize: () => true });
  const res = await call(t.POST, 'delete-everything', {});
...
test('a body that is not JSON is a 400, not a crash', async () => {
  const t = await build({ authorize: () => true });
  const res = await call(t.POST, 'check', 'not json at all');
...
test('a signed webhook verifies and reaches onEvent', async () => {
  ...
  const res = await call(t.POST, 'webhook', evt, sign(evt, now()));
...
test('a forged signature is 400 and the handler never runs', async () => {
  const seen = [];
  const t = await build({ authorize: () => true, webhookSecret: SECRET, onEvent: (e) => seen.push(e) });
  const res = await call(t.POST, 'webhook', evt, {
...
test('a stale timestamp is rejected as a replay', async () => {
  ...
  const res = await call(t.POST, 'webhook', evt, sign(evt, now() - 3600));
...
test('a webhook body is never re-serialised before verification', async () => {
  /* Signatures cover the exact bytes. A payload whose key order or spacing
     would not survive JSON.parse → JSON.stringify proves the route reads raw
     text rather than round-tripping through an object. */
  const odd = '{"event":"verification.flagged",  "id":"evt_2",\n"data":{"phone":"+212600000000"}}';
  const seen = [];
  const t = await build({ authorize: () => true, webhookSecret: SECRET, onEvent: (e) => seen.push(e) });
  const res = await call(t.POST, 'webhook', odd, sign(odd, now()));
...
test('without a webhookSecret the webhook segment is a 404', async () => {
  const t = await build({ authorize: () => true });
  const res = await call(t.POST, 'webhook', evt, sign(evt, now()));
