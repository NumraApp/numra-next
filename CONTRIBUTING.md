...
`node:test` runner. Nothing here boots Next: a route handler takes a
Web-standard `Request` and returns a Web-standard `Response`, so the tests
call it directly, and `test/mock-server.js` stands in for the API. That is
also why `next` is an optional peer dependency rather than a hard one.
...
`test/route.test.js` covers the two things this adapter can get wrong on its
own: reading the action out of `params` so the catch-all works at any mount
point and under any segment name, and reading the webhook body with
`request.text()` so the signature is checked against the exact bytes.
...
Express, Fastify and Nuxt packages.
...
Open an issue with the package version, the Next version, the Node version,
